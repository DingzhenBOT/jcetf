...
def _signal(tier="SMALL_POSITION", score=78.0, confidence=85, regime="TREND_UP"):
    return {
        "target_etf": "510300",
        "signal_type": tier,
        "score": score,
        ...
        "suggested_position_range": [10, 25],
        "supporting_metrics": {
            "etf_rsi14": 55,
            "etf_rs_20d": 1.05,
            "sector_score": 70,
...
def test_no_llm_called_by_default():
    ...
    assert "禁止追高" in out["content"]


def test_position_text_present():
    out = OpinionEngine().generate(_signal(tier="OBSERVE", score=65, confidence=70), "post_close", {})
