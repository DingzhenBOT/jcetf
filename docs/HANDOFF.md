...
## 2026-08-01 当前覆盖说明（优先于下方历史轮次）

- 策略规则已升级到 v3.0：市场方向只通过 `market_score` 进入综合分一次，`WEAK/BEAR` 不再在档位层重复扣分，也不再让持仓分析全体 RECONFIRM/EXIT。
- 资金流只有在同一来源至少存在 3 个真实 `main_net_inflow` 观测时才参与评分；全空列或 1–2 条异动榜数据按缺失处理，权重重归一化并降低置信度。
- 基础档位、最终档位、有效组件权重和调档原因写入 `supporting_metrics`；ATR/看空量价等软风险合计最多降一档。
- ETF/信号列表移除重复的“市场环境”列；详情分析仍保留市场作为评分背景。
- `docs/trading_methodology.md` 已按当前实现重写相关章节；下方 C23/C25 中关于 `BEAR −18 / WEAK −10`、资金流空值评分、全局 BEAR 持仓退出和 A1–A10 未修复的描述均为历史记录，不得作为当前实现依据。
- 生产数据事实：完整覆盖日 2026-07-30 的 48 支 ETF 基础分均低于 60（最高约 54.24），所以全为 `NO_PARTICIPATE` 是当日因子结果，不是市场标签硬压；2026-07-31 日线覆盖不足，收盘流水线必须继续锚定 7/30，不可混用日期强行生成。
