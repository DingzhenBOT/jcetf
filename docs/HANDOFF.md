...
- ✅ **东财 push2 直连**（`push2.eastmoney.com`，**CVM 实测可达**，绕过被墙的 akshare em / push2his）：`backend/app/data_provider/eastmoney_web.py` —— `fetch_sector_fund_flow_snapshot`（clist 一次性拉全板块当日主力净流入/涨跌幅）+ `fetch_sector_kline`（secid=90.BKxxxx 板块日K）；collector 的 `collect_sector_history_web` / `collect_sector_fund_flow_web` 入库 `SECTOR/BAR/em_web`，已在 C19-F 消除板块降级（详见 devlog C19 F）。**沙箱 push2 被 RST，仅 CVM 能真实拉取**；测试用 mock 验证解析。
...
- `backend/app/data_provider/eastmoney_web.py` —— **C19-F 新增**：东方财富 push2 直连源（CVM 可达，绕过被墙 em/push2his）。`fetch_sector_fund_flow_snapshot`（clist 全板块资金流/涨跌）、`fetch_sector_kline`（secid=90.BKxxxx 板块日K）；异常抛 RuntimeError 交由 collector `*_web` 方法降级。
...
8. **板块历史/资金流在 CVM 原 D4 降级已消除（C19-F）**：原因为 em(push2his) 被 RST 拦截 + ths 解析报错/返回空 + `get_sector_fund_flow_history` 根本没接 ths → 板块数据在 CVM 完全缺失。C19-F 经 CVM 实测 `push2.eastmoney.com` 可达，新增 `eastmoney_web.py` 直连（clist 资金流 + kline 日K），`collect_sector_history_web`/`collect_sector_fund_flow_web` 入库 `SECTOR/BAR/em_web`，降级已消除。**沙箱 push2 被 RST，仅 CVM 能真实拉取**；测试用 mock 验证解析。**后台仍保留 ths 兜底**：`get_sector_history` 走 ths 对部分板块覆盖（军工/新能源车/5G/证券/银行/白酒），与 em_web 互为补充。注：腾讯 `web.ifzq.gtimg.cn` K 线仍不支持板块 BK 代码（仅指数/个股），gtimg 不作板块历史源。
