...
// 顶部指数带：上证指数（000001）作 hero 突出，其余指数红涨绿跌 + 百分比 + 摆锤图。
  ...
  >
    <div class="flex items-stretch overflow-x-auto no-scrollbar">
      ...
      <button
        v-if="hero"
        type="button"
        class="group flex-shrink-0 w-[210px] px-5 py-3 text-left border-r border-slate-200 hover:bg-white/70 active:scale-[0.99] transition focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-300"
      ...
      >
        ...
        <PendulumChart :change-percent="hero.change_percent" :max="4" height="78px" />
      </button>
      ...
      <button
        ...
        :key="it.code"
        type="button"
        class="group flex-shrink-0 w-[150px] px-4 py-3 text-left border-r border-slate-100 hover:bg-white/70 active:scale-[0.99] transition focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-300"
      ...
      >
        ...
        <PendulumChart :change-percent="it.change_percent" :max="4" height="64px" />
      </button>
