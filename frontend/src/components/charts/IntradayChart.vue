...
// 盘中分时图（同花顺风格，连续轴）：
// - x 轴 09:30–11:30 直接接 13:00–15:00（午休处连续、不插空槽、不断开），贴合同花顺分时观感。
// - y 轴为「当日涨跌幅%」，0% 固定在中线（对称 min/max），红涨绿跌语义由成交量柱体现。
// - 价格线白色（#fff），均价线黄色（#f5c518，分时均价 = 累计成交额/累计成交量 VWAP），共享同一 y 轴。
// - 底部成交量与价格共享同一 x 轴；净买入（本分钟价 > 上一分钟价）红、净卖出绿、持平灰。
...
// 交易时段类目：上午 09:30-11:30 直接接下午 13:00-15:00（连续轴，午休处不插空槽）。
...
const option = computed<EChartsOption>(() => {
  ...
  // 成交量着色：本分钟价 vs 上一分钟价（首分钟 vs 昨收；13:00 跨午休回退 11:30）
...
// 取某标签对应「上一分钟」价格（用于成交量净买/净卖着色）；13:00 跨午休回退 11:30，首分钟回退昨收。
function prevPriceByLabel(map: Map<string, number>, label: string): number | null {
  ...
  // 跨上午/下午边界（13:00 无 12:59）回退同段首或昨收
